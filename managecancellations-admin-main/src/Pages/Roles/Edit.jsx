...
function EditRoles() {
  ...
  useEffect(() => {
    ...
    if (!accessRight?.is_edit) {
      navigate("/dashboard");
    }
    let url = window.location.href;
    let ID = url.substring(url.lastIndexOf("/") + 1);
    configuration.getAPIaxios({ url: 'admin/role/list', params: { ID } }).then((data) => {
      if (data) {
        setFields(data)
      }
    }).catch(error => {
  ...
  const handleSubmit = () => {
    if (validation()) {
      configuration.allAPI({ url: 'admin/role/update', params: fields, method: 'PATCH' }).then(async (data) => {
        if (data.payload) {
  ...
  return (
    <>
      <Breadcrumb pages={breadCrumbLinks} pagetitle="Edit Role" />

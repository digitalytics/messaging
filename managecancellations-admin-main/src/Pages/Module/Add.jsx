...
function AddModule() {
  ...
  const breadCrumbLinks = [
    ...
    { name: "Add Module", href: "#", current: true },
  ];
  ...
  const handleSubmit = () => {
    if (validation()) {
      configuration
        .allAPI({ url: "admin/module/create", params: fields, method: "PUT" })
        .then(async (data) => {
          if (data) {
  ...
  return (
    <>
      <Breadcrumb pages={breadCrumbLinks} pagetitle="Add Module" />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                  <div>
                    ...
                    <div className="mt-2">
                      ...
                      />
                      {errors?.title ? <Label className={`block text-sm text-[#D80027]`} text={errors?.title} /> : null}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="mt-2">
                      <Input
                        ...
                        className="block w-full appearance-none text-black bg-white rounded-[5px] border-[#EFEFF4]  focus:border-[#EFEFF4] focus:ring-[#B4FA64] focus:ring-1  px-3 py-2.5 placeholder-gray-400  text-base"
                      />
                      {errors?.code ? <Label className={`block text-sm text-[#D80027]`} text={errors?.code} /> : null}
                    </div>
                  </div>
                </div>
